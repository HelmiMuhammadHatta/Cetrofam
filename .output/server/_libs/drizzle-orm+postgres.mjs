	...
	*/
	...
	buildSelectQuery({ withList, fields, fieldsFlat, where, having, table, joins, orderBy, groupBy, limit, offset, lockingClause, distinct, setOperators }) {
		...
		if (lockingClause) {
			...
			if (lockingClause.config.noWait) clauseSql.append(sql` no wait`);
...
var PgSelectBuilder = class {
	...
	*/
	from(source) {
		...
		else if (is(source, PgViewBase)) fields = source[ViewBaseConfig].selectedFields;
...
var PgUpdateBuilder = class {
	...
	set(values) {
		return this.authToken === void 0 ? new PgUpdateBase(this.table, mapUpdateSet(this.table, values), this.session, this.dialect, this.withList) : new PgUpdateBase(this.table, mapUpdateSet(this.table, values), this.session, this.dialect, this.withList).setToken(this.authToken);
	}
};
...
function construct(client, config = {}) {
	...
	const db = new PostgresJsDatabase(dialect, new PostgresJsSession(client, dialect, schema, { logger }), schema);
	db.$client = client;
	return db;
}
...
//#endregion
export { serial as a, text as i, pgTable as n, src_default as o, timestamp as r, drizzle as t };
